...
class Webserver {
    ...
    private inner class ClientHandler(private val clientSocket: Socket) :
        ...
        private fun handle(msg: String) {
            ...
            when (command) {
                org.academiadecodigo.www.command.Commands.PM -> sendPrivate(
                    commandParser.getActionCommand(),
                    commandParser.getText()
                )
         ...
         * @param msg                       String
         */
        private fun sendPrivate(username: Unit, msg: Unit) {
            for (ch in list!!) {
...
private fun Nothing?.getOutputStream(): OutputStream {
